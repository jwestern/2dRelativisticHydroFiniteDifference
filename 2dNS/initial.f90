	...
	subroutine initial(Uold,dx,dy,x,y,Nv,N)
	use m_pars, only : ivort,resume,resume_integer,weos
...
!	      uold(ivort,:,i) =  0.2*sin(20.*ppi/y(3,N)*y(1,i))* &
...
if (resume==1) then
	...
	ret = gft_read_brief('vort',tslice,uold(ivort,:,:))
